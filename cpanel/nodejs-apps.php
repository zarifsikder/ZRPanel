...
function node_is_running($pid_file) {
    if (!file_exists($pid_file)) return false;
    $pid = (int)trim(file_get_contents($pid_file));
    if ($pid <= 0) return false;
    $result = @file_get_contents("/proc/{$pid}/cmdline");
    if ($result !== false && (strpos($result, 'node') !== false || strpos($result, 'npm') !== false)) return true;
    @unlink($pid_file);
    return false;
}

function node_start_app($app, $home_dir) {
    $pid_dir = $home_dir . '/.nodeapps';
    ...
    if (!empty($app['env_vars'])) {
        foreach (explode("\n", $app['env_vars']) as $line) {
            $line = trim($line);
            if (!empty($line) && strpos($line, '=') !== false) {
                $parts = explode('=', $line, 2);
                $env_str .= ' ' . escapeshellarg(trim($parts[0])) . '=' . escapeshellarg(trim($parts[1]));
            }
        }
    }

    $cmd = "cd " . escapeshellarg($app_dir) . " && PORT=" . (int)$app['port'] . $env_str . " setsid nohup sh -c " . escapeshellarg($app['start_command'] ?? 'node index.js') . " < /dev/null > " . escapeshellarg($log_file) . " 2>&1 & echo $!";
...
function node_stop_app($pid_file) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $port = max(1024, min(65535, (int)($_POST['port'] ?? 3000)));
        $node_version = preg_replace('/[^0-9.]/', '', $_POST['node_version'] ?? '18');
        $root_dir = trim($_POST['root_dir'] ?? '');
        $start_command = trim($_POST['start_command'] ?? 'node index.js');
        ...
        if (empty($app_name) || empty($domain) || empty($root_dir)) {
            ...
            redirect('/cpanel/nodejs-apps.php');
        }

        $exists = $db->prepare("SELECT id FROM nodejs_apps WHERE app_name = ? AND user_id = ?");
        ...
        $port_check = $db->prepare("SELECT id FROM nodejs_apps WHERE port = ? AND user_id = ?");
        $port_check->execute([$port, $user_id]);
        if ($port_check->fetch()) {
            flash('error', "Port {$port} is already in use by another app");
            redirect('/cpanel/nodejs-apps.php');
        }

        $db->prepare("INSERT INTO nodejs_apps (user_id, app_name, domain, port, node_version, root_dir, start_command, env_vars, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'stopped')")
           ->execute([$user_id, $app_name, $domain, $port, $node_version, $root_dir, $start_command, $env_vars]);

        flash('success', "Node.js app '{$app_name}' deployed successfully");
        ...
        $db->prepare("UPDATE nodejs_apps SET domain = ?, port = ?, node_version = ?, root_dir = ?, start_command = ?, env_vars = ? WHERE id = ? AND user_id = ?")
           ->execute([$domain, $port, $node_version, $root_dir, $start_command, $env_vars, $id, $user_id]);
        flash('success', 'App settings updated');
    } elseif ($action === 'start') {
        $id = (int)($_POST['id'] ?? 0);
        $stmt = $db->prepare("SELECT * FROM nodejs_apps WHERE id = ? AND user_id = ?");
        $stmt->execute([$id, $user_id]);
        $app = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($app) {
            if (node_start_app($app, $home_dir)) {
                $db->prepare("UPDATE nodejs_apps SET status = 'running' WHERE id = ?")->execute([$id]);
                flash('success', 'Application started (PID: ' . trim(file_get_contents($home_dir . '/.nodeapps/' . $id . '.pid')) . ')');
            } else {
        ...
        $id = (int)($_POST['id'] ?? 0);
        node_stop_app($home_dir . '/.nodeapps/' . $id . '.pid');
        $db->prepare("UPDATE nodejs_apps SET status = 'stopped' WHERE id = ? AND user_id = ?")->execute([$id, $user_id]);
        ...
        $stmt->execute([$id, $user_id]);
        $app = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($app && node_start_app($app, $home_dir)) {
            $db->prepare("UPDATE nodejs_apps SET status = 'running' WHERE id = ?")->execute([$id]);
            flash('success', 'Application restarted');
        } else {
            $db->prepare("UPDATE nodejs_apps SET status = 'error' WHERE id = ?")->execute([$id]);
            flash('error', 'Restart failed');
        }
    } elseif ($action === 'delete') {
        $id = (int)($_POST['id'] ?? 0);
        node_stop_app($home_dir . '/.nodeapps/' . $id . '.pid');
        $del = $db->prepare("SELECT app_name FROM nodejs_apps WHERE id = ? AND user_id = ?");
        $del->execute([$id, $user_id]);
        $app = $del->fetch(PDO::FETCH_ASSOC);
        if ($app) {
            $db->prepare("DELETE FROM nodejs_apps WHERE id = ? AND user_id = ?")->execute([$id, $user_id]);
...
$apps = $db->prepare("SELECT * FROM nodejs_apps WHERE user_id = ? ORDER BY created_at DESC");
$apps->execute([$user_id]);
$apps = $apps->fetchAll(PDO::FETCH_ASSOC);

foreach ($apps as &$app) {
    $pid_file = $home_dir . '/.nodeapps/' . $app['id'] . '.pid';
    $running = node_is_running($pid_file);
    $actual_status = $running ? 'running' : 'stopped';
    if ($app['status'] !== $actual_status) {
        $db->prepare("UPDATE nodejs_apps SET status = ? WHERE id = ?")->execute([$actual_status, $app['id']]);
...
foreach ($apps as $a) {
    ...
    elseif ($a['status'] === 'error') $apps_error++;
    else $apps_stopped++;
}
...
if (isset($_GET['logs']) && $_GET['logs'] !== '') {
    ...
    $log_stmt = $db->prepare("SELECT app_name FROM nodejs_apps WHERE id = ? AND user_id = ?");
...
if (isset($_GET['edit']) && $_GET['edit'] !== '') {
    ...
    $edit_stmt = $db->prepare("SELECT * FROM nodejs_apps WHERE id = ? AND user_id = ?");
...
<div class="page-hero fade-in">
    <div class="hero-icon green"><i data-lucide="rocket" class="lucide"></i></div>
    <div class="hero-text">
        <div class="hero-title">Node.js Applications</div>
        <div class="hero-desc">Deploy and manage Node.js apps as background processes with PID tracking and live log capture.</div>
    </div>
...
<div class="stats-grid fade-in-delay-1">
    ...
    <div class="stat-card stat-red fade-in">
        <div class="stat-icon"><i data-lucide="alert-triangle" class="lucide"></i></div>
        <div class="stat-info">
            <div class="stat-number"><?= $apps_error ?></div>
            <div class="stat-label">Errored</div>
            <div style="font-size:11px;color:var(--text4);margin-top:2px">failed to launch</div>
        </div>
    </div>
</div>
    ...
    <div class="card-body">
        <form method="POST" class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="text" name="app_name" required placeholder="e.g. my-api" pattern="[a-zA-Z0-9_-]{2,50}">
            </div>
            <div class="form-group">
                <label>Domain</label>
            ...
            <div class="form-group">
                ...
                <select name="node_version">
                    <?php foreach (['14','16','18','20','22'] as $v): ?>
                        <option value="<?= $v ?>" <?= $v === '18' ? 'selected' : '' ?>>Node.js <?= $v ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group">
                <label>Root Directory</label>
                <input type="text" name="root_dir" required placeholder="e.g. public_html/my-app" value="public_html/">
            </div>
            <div class="form-group">
                <label>Start Command</label>
                <input type="text" name="start_command" value="node index.js" placeholder="node index.js">
            </div>
            <div class="form-group" style="grid-column:1/-1">
                <label>Environment Variables <span style="color:var(--text3);font-size:12px">(one per line, KEY=value)</span></label>
                <textarea name="env_vars" rows="4" placeholder="NODE_ENV=production&#10;PORT=3000&#10;DB_HOST=localhost" style="width:100%;font-family:monospace;font-size:13px;background:var(--bg4);color:var(--text);border:1.5px solid var(--border);border-radius:var(--radius-sm);padding:12px;resize:vertical"></textarea>
            </div>
            <div class="form-group" style="grid-column:1/-1">
...
<div class="card fade-in" style="margin-bottom:20px">
    <div class="card-header">
        ...
        <a href="/cpanel/nodejs-apps.php" class="btn btn-sm btn-danger" style="text-decoration:none;margin-left:auto"><i data-lucide="x" class="lucide" style="width:12px;height:12px"></i> Close</a>
    </div>
    <div class="card-body" style="padding:0">
        <div style="background:#0d1117;color:#2ecc71;font-family:monospace;font-size:12px;padding:16px;max-height:400px;overflow-y:auto;white-space:pre-wrap;word-break:break-all;line-height:1.7"><?= $logs_content ?: '<span style="color:#666">No log output yet</span>' ?></div>
    </div>
</div>
<?php endif; ?>
...
<div class="card fade-in" style="margin-bottom:20px;border:1.5px solid var(--accent)">
    <div class="card-header">
        ...
        <a href="/cpanel/nodejs-apps.php" class="btn btn-sm btn-danger" style="text-decoration:none;margin-left:auto"><i data-lucide="x" class="lucide" style="width:12px;height:12px"></i> Cancel</a>
    </div>
    <div class="card-body">
        <form method="POST" class="form-grid">
            ...
            <div class="form-group">
                ...
                <select name="node_version">
                    <?php foreach (['14','16','18','20','22'] as $v): ?>
                        <option value="<?= $v ?>" <?= $v === $edit_app['node_version'] ? 'selected' : '' ?>>Node.js <?= $v ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label>Start Command</label>
                <input type="text" name="start_command" value="<?= h($edit_app['start_command']) ?>">
            </div>
            <div class="form-group" style="grid-column:1/-1">
                <label>Environment Variables</label>
                <textarea name="env_vars" rows="4" style="width:100%;font-family:monospace;font-size:13px;background:var(--bg4);color:var(--text);border:1.5px solid var(--border);border-radius:var(--radius-sm);padding:12px;resize:vertical"><?= h($edit_app['env_vars']) ?></textarea>
            </div>
            <div class="form-group" style="grid-column:1/-1">
                <button type="submit" class="btn btn-primary" style="display:inline-flex;align-items:center;gap:6px"><i data-lucide="save" class="lucide" style="width:16px;height:16px"></i> Save Changes</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="card-body" style="padding:<?= empty($apps) ? '14px' : '16px' ?>">
        <?php if (empty($apps)): ?>
            <div class="empty-state" style="padding:10px 0 18px">
                ...
                <strong>No Node.js applications deployed yet</strong>
                <p>Deploy your first app above &mdash; it runs as a background process with its own PID and log file.</p>
            </div>
        <?php else: ?>
        <div class="app-list">
            <?php foreach ($apps as $app): ?>
                <div class="app-row" data-name="<?= h(strtolower($app['app_name'] . ' ' . $app['domain'] . ' ' . $app['port'] . ' ' . $app['node_version'] . ' ' . $app['status'])) ?>">
                    <span class="app-ic <?= $app['status'] === 'running' ? '' : ($app['status'] === 'error' ? 'is-err' : 'is-off') ?>"><i data-lucide="terminal" class="lucide"></i></span>
                    <div class="app-main">
                        <div class="app-name">
                            ...
                            <?php elseif ($app['status'] === 'error'): ?>
                                <span class="badge badge-suspended" style="display:inline-flex;align-items:center;gap:5px"><span class="dot red"></span> Error</span>
                            <?php else: ?>
                        ...
                        <div class="app-meta">
                            ...
                            <span class="badge badge-blue" style="font-size:10px">Node v<?= h($app['node_version']) ?></span>
...
.app-ic{width:40px;height:40px;border-radius:10px;flex-shrink:0;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#047857,#059669 55%,#10b981);color:#fff;box-shadow:0 4px 10px rgba(5,150,105,.18)}
.app-ic.is-off{background:linear-gradient(135deg,#78716c,#a8a29e 55%,#d6d3d1);box-shadow:0 4px 10px rgba(168,162,158,.18)}
.app-ic.is-err{background:linear-gradient(135deg,#b91c1c,#dc2626 55%,#ef4444);box-shadow:0 4px 10px rgba(220,38,38,.18)}
