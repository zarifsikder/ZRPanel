...
ok "Exporting the initial DNS zone files (dns_sync.php)…"
if [ -f "$PANEL_DIR/dns_sync.php" ]; then
    if php "$PANEL_DIR/dns_sync.php" >"$LOG_DIR/dns.log" 2>&1; then
        info "Zones exported → \$HOME/.nsd/zones (served by scripts/dns_server.php, ZR_DNS_PORT default 5390)"
    else
        warn "Initial zone export failed — see $LOG_DIR/dns.log"
    fi
fi
...
printf '  %-13s  %s\n' "DNS"        "udp://0.0.0.0:${ZR_DNS_PORT:-5390}  ${c_dim}(local authoritative; zone files in ~/.nsd)${c_reset}"
printf '  %-13s  %s\n' "Launchers"  "~/start  ·  ~/off"
